...
export default function BorderlessBox({
  ...
  // Generate days array with full date information
  const days: DayInfo[] = (() => {
  ...
  const [editingTaskData, setEditingTaskData] = useState<TaskTimeData | null>(null)
  ...
  useEffect(() => {
    const handleResize = () => {
      if (containerRef.current) {
        ...
        const availableHeight = window.innerHeight - headerHeight - safetyMargin
  ...
  const isTimeSlotOccupied = (day: string, hour: number, taskId?: string) => {
    return scheduledTasks.some(
      (task) =>
        task.day === day && task.startHour <= hour && task.endHour > hour && (taskId ? task.taskId !== taskId : true),
    )
  }

  // Check if a range of time slots is available
  const areTimeSlotsAvailable = (day: string, startHour: number, endHour: number, taskId?: string) => {
  ...
  const handleDrop = (e: React.DragEvent, fullDate: string, hour: number) => {
    ...
    try {
      ...
      if (taskData) {
        ...
        const existingTask = scheduledTasks.find((t) => t.taskId === task.id)?.task || {
          id: task.id,
  ...
  const handleEditTask = (taskData: TaskTimeData) => {
    const scheduledTask = scheduledTasks.find((task) => task.taskId === taskData.id)
  ...
  const handleTaskFormSubmit = (formData: {
    ...
    if (editingTaskData && editingTaskData.id) {
      const existingTaskIndex = scheduledTasks.findIndex((task) => task.taskId === editingTaskData.id)
      ...
      if (existingTaskIndex !== -1) {
        if (onEditTask) {
          ...
          onEditTask({
            ...
            startHour:
              formData.startHour !== undefined ? formData.startHour : scheduledTasks[existingTaskIndex].startHour,
            endHour: formData.endHour !== undefined ? formData.endHour : scheduledTasks[existingTaskIndex].endHour,
            day: formData.day !== undefined ? formData.day : scheduledTasks[existingTaskIndex].day,
            task: updatedTask,
          })
        }
      } else {
        if (editingTaskData.id) {
          const startHour = formData.startHour !== undefined ? formData.startHour : editingTaskData.startHour
          const endHour = formData.endHour !== undefined ? formData.endHour : editingTaskData.endHour
          const day = formData.day !== undefined ? formData.day : editingTaskData.day
  ...
  const getTimeSlots = (fullDate: string): TimeSlot[] => {
    ...
    for (let hour = 1; hour <= 24; hour++) {
      ...
      const tasksStartingInThisHour = dayTasks.filter((task) => Math.floor(task.startHour) === hour)

      // Find tasks that overlap this hour but started earlier
      const overlappingTasks = dayTasks.filter((task) => task.startHour < hour && task.endHour > hour)

      // Find tasks that end in this hour but started earlier
      const tasksEndingInThisHour = dayTasks.filter(
        (task) => Math.floor(task.startHour) < hour && Math.floor(task.endHour) === hour,
      )
      ...
      <div className="grid grid-cols-3 divide-x divide-gray-300 dark:divide-gray-700">
        {days.map(({ dayName, dayNumber, fullDate }) => (
          <div key={fullDate} className="flex flex-col">
            <h2 className="text-center text-[18px] dark:text-gray-200">{dayName}</h2>
            <h2 className="text-center font-semibold text-[32px] leading-none dark:text-gray-100">{dayNumber}</h2>

            <div className="flex flex-col flex-1">
              {getTimeSlots(fullDate).map((slot) => {
                const isErrorSlot = dropError?.day === fullDate && dropError?.hour === slot.startHour
                ...
                return (
                  <div
                    ...
                    onDrop={!hasTask ? (e) => handleDrop(e, fullDate, slot.startHour) : undefined}
                  >
                    ...
                    {slot.tasks.length > 0 && (
                      <ScheduledTaskList
                        ...
                        onTaskResize={(taskId, newEndHour) => {
                          if (areTimeSlotsAvailable(fullDate, slot.startHour + 1, newEndHour, taskId)) {
                            onTaskResize(taskId, newEndHour)
                          }
                        }}
                      />
