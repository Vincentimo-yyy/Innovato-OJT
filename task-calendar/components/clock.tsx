...
const Clock: React.FC<ClockProps> = ({ className = "", is24Hour }) => {
  ...
  useEffect(() => {
    const updateClock = () => {
      ...
      if (is24Hour) {
        ...
        setPeriod(""); // No AM/PM in 24-hour format
      } else {
        // 12-hour format
  ...
  }, [is24Hour]); // Re-run effect when format changes
  ...
  return (
    <div
      className={`flex flex-row items-center gap-3 p-3 bg-gray-200 rounded-lg shadow-md max-h-[90px] ${className}`}
    >
